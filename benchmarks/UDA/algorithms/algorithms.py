...
from models.models import classifier, ReverseLayerF, Discriminator, RandomLayer, Discriminator_CDAN, \
    codats_classifier, AdvSKM_Disc, CNN_ATTN, CLS
...
from algorithms.uda_algorithms import * #OSBP, UniOT
...
class CDAN(Algorithm):
    ...
    def __init__(self, backbone, configs, hparams, device):
        ...
        self.random_layer = RandomLayer([configs.in_dim, configs.n_share+configs.n_src_private],
                                        configs.in_dim)
        self.configs = configs
        self.optimizer_disc = torch.optim.Adam(
            self.domain_classifier.parameters(),
...
class DSAN(Algorithm):
    ...
    def __init__(self, backbone, configs, hparams, device):
        ...
        # Alignment losses
        self.loss_LMMD = LMMD_loss(device=device, class_num=configs.n_share + configs.n_src_private).to(device)
...
class DeepJDOT(Algorithm):
    ...
    def classifier_cat_loss(self, src_y, trg_pred):
        label_loss = torch.matmul(src_y.float(), self.softmax(trg_pred).T)
        return torch.sum(self.gamma * label_loss)

    def align_loss(self, src_feat, trg_feat):
        # gdist = self.L2_dist(src_feat, trg_feat)
        gdist = torch.matmul(F.normalize(src_feat), F.normalize(trg_feat).t())
    ...
    def training_epoch(self, src_loader, trg_loader, avg_meter, epoch):
        ...
        for step, ((src_x, src_y), (trg_x, _)) in joint_loader:
            ...
            with torch.no_grad():
                C0 = torch.matmul(F.normalize(src_feat), F.normalize(trg_feat).T)
                C1 = torch.matmul(src_y.float(), self.softmax(trg_pred).T)
